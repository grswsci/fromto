...
driverpubmed <- function(...,
  ...
  for (var_url in url) {
    ...
    # 尝试读取网页并提取摘要
    webpage = tryCatch({
      random_delay = runif(1, min = 3, max = 5)
      Sys.sleep(random_delay)
      read_html(httr::GET(var_url, httr::user_agent("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.3")))
      #read_html(var_url)
    }, error = function(e) {
    ...
    abstract_text = gsub("\n","",abstract_text)
    abstract_text = gsub("Abstract ","Abstract: ",abstract_text)
    abstract_text = gsub("  ","",abstract_text)
    abstract_text = paste0(abstract_text,paste0("[",match(var_url,url),"]"))
  ...
  res2 = data.frame(uid = paste0("https://www.ncbi.nlm.nih.gov/pubmed/?term=",uid),
                    pubdate = pubdate,
                    firstauthor = sortfirstauthor,
                    lastauthor = lastauthor,
                    title = title,
                    ...
                    #DataBase = createLink(paste0("https://grswsci.top/"),"光热生物数据库\n微信：bioinformaticsboy"),
                    stringsAsFactors = F)
  res2$Cite = paste0("[",1:nrow(res2),"]")
  write.table(res2,paste0("output_paper.txt"),row.names = F,quote = F,sep = "\t")
}
...
driverpubmed3 <- function(...,
                         year_start = "2021",
                         year_end = "2024",
                         name_use = "output_paper"){
  createLink <- function(base,val) {
    sprintf('<a href="%s" class="btn btn-link" target="_blank" >%s</a>',base,val)
  }
  createLink_doi <- function(base,val) {
  ...
  res = na.omit(res)
  y = DT::datatable(res,escape = F,rownames=F)
  DT::saveWidget(y,paste0(name_use,".html"),selfcontained = TRUE)
  res2 = data.frame(uid = paste0("https://www.ncbi.nlm.nih.gov/pubmed/?term=",uid),
                    pubdate = pubdate,
                    firstauthor = sortfirstauthor,
                    lastauthor = lastauthor,
                    ...
                    stringsAsFactors = F)
  res2$Cite = paste0("[",1:nrow(res2),"]")
  write.table(res2,paste0(name_use,".txt"),row.names = F,quote = F,sep = "\t")
}
